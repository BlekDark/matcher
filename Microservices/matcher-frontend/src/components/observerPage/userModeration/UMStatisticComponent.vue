<template>
    ...
    >
      <div class="pairs-block">
        <div v-if="sportFilter.allSport" class="original-comparison">
          <div v-for="run in this.filteredRuns" class="run">
            <div class="sport-title">
              {{ sportName(run.sport_id) }}
            </div>

            <div v-if="run.results.length > 0">
              <div v-for="result in run.results">
                <div v-if="!result.mismatch && !result.is_match" class="field-items">
            ...
            <div v-else style="text-align: center; margin-bottom: 20px; margin-left: 10px; margin-right: 10px">По данному вида спорта пользователь не отвергал несматченные пары и не матчил несматченные системой</div>
          </div>
        </div>
        ...
        <div v-else class="original-comparison">
          <div v-for="run in this.filteredRuns" class="run">
            <div v-if="sportFilter.sport === run.sport_id" >
              ...
              <div v-else style="text-align: center; margin-bottom: 20px">По данному вида спорта пользователь не отвергал несматченные пары и не матчил несматченные системой</div>
            </div>
          </div>
        </div>
  ...
  <!--          <div v-if="this.pairDef.types && sportFilter.allSport && this.undefinedSportConfig().length > 0"-->
  <!--               class="add-block"-->
  <!--          >-->
  <!--            <el-select-->
  ...
  watch: {
    selectedTask:{
          ...
          handler (newVal) {
            ...
            if (this.runs){
              this.filteredRuns = this.runs.map((run) => {
                const filtered = run.results.filter((result) => {
                  return (result.is_match && result.mismatch) || (!result.is_match && !result.mismatch);
                });
